   ...
   script, loaded in order with its siblings and sharing one global scope. */
    ...
    function renderWeekDeposit() {
            ...
            return `
                <div class="panel collapsed" style="margin-bottom:8px;">
                    <div class="panel-head" onclick="toggleCollapse(this)">
                        <span style="font-size:13px;"><span class="type-pill">${c.charge_id}</span> <b>${escHtml(empName(c.employee_id))}</b> · ${statusBadge(c.status)}</span>
        ...
        statusesPresent.forEach(status => {
            ...
            rows += `<div class="detail-subhead" style="margin:12px 0 6px; cursor:pointer;" onclick="toggleStatusGroup('weekdeposit','${status}')"><span class="rec-caret">${collapsed ? '▸' : '▾'}</span> ${statusBadge(status)} <span style="color:var(--text-muted); font-weight:400; text-transform:none; letter-spacing:normal;">(${group.length})</span></div>`;
            if (collapsed) return;
