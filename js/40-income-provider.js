    ...
    function renderStatement() {
        ...
        let html = `
            <div class="panel">
                ...
                <div style="font-size:12px;color:var(--text-muted);margin-bottom:10px;">${enumLabel(emp.person_type)} · <span class="status-badge ${emp.status==='Active'?'status-active':'status-quit'}">${enumLabel(emp.status)}</span>${emp.department ? ' · ' + escHtml(emp.department) : ''}</div>
            </div>
            <div class="panel">
