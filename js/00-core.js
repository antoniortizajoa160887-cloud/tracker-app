/* Tracker app — part 1/8: 00-core.js
   ...
   script, loaded in order with its siblings and sharing one global scope. */
    ...
    const CHANGELOG = [
        { version: 'v3.82', date: '2026-08-22', notes: 'Messages redesigned: the chat now fills the whole working area like every other screen instead of a small box. Conversations and the chat header show the person\'s profile photo (or their colored initials). On phones the list and the open chat are two full screens with a ← button to go back, like a real messaging app. The composer is a round pill bar, and the emoji picker is now a compact WhatsApp-style popover — category tabs on top, search underneath, and a dense grid of properly sized emojis — on both PC and mobile. Starting and switching conversations was rebuilt too: "+" now opens a searchable people list with photos (tap a person to chat), and the "⌄" in the chat header lists every conversation with that person plus one-tap starters for their claims, charges, income records or a missing day.' },
        { version: 'v3.81', date: '2026-08-22', notes: 'Visual refresh for Route Tracker, Daily Report and Daily Pay: the report filters (slicers) lose the old Excel look — quiet cards, small-caps headers, navy selection — and the +/- buttons match the app style. The Route Tracker and Daily Report tables now keep their header row pinned while you scroll (and the report keeps its totals row pinned at the bottom), with row highlighting on hover. Daily Pay cards drop the solid blue banner for a quiet header, and on PC the seven days show as one row per person.' },
        ...
        { version: 'v3.76', date: '2026-08-22', notes: 'Compact People sections (4.1 layout, continued): Employees and Users now open with the slim section bar. On Employees, the roster is first on screen — the New Contact form hides behind "+ New Contact" (editing someone still opens it automatically), Export/Import CSV moved into the "⋯" menu, and sorting sits on the search line. On Users, the user list is now open by default and the Create User form hides behind "+ New User"; the security panels (active sessions, failed sign-ins, sign-out history) stay collapsed as before.' },
        { version: 'v3.75', date: '2026-08-22', notes: 'Compact Data Sync (4.1 layout, continued): the tab now opens with a slim bar holding the title and both export buttons, with the export details right below. The two import panels start collapsed — tap a title to open one. The Administrator and Super Admin danger zones are now quiet, collapsed accordions: no red wall of delete buttons on screen until you deliberately tap the section title. Every action is unchanged — just tucked away until asked for.' },
        { version: 'v3.74', date: '2026-08-22', notes: 'Compact Finance sections (first round of the approved 4.1 layout): Claims & Charges, Income, Invoices and Bills each open with a slim section bar — title, live totals as chips, and the "+ New" buttons — so the records list is visible right away. The create forms are hidden until you tap "+ New" (tap again to close; editing a record still opens the right form automatically). All the Export / Report / Import buttons moved into a "⋯" menu in the bar, and the sort control now sits on the same line as search and filters. Every button and function is unchanged — just repositioned.' },
        { version: 'v3.73', date: '2026-08-22', notes: 'Bug fix: saving a new claim, charge, income or vehicle could fail with a "duplicate key" error. The next record ID was computed by counting the existing records — so after any record was ever deleted, the count fell behind the highest ID still in use and the app tried to reuse an existing number. The next ID is now always the highest existing number plus one, so saves work again (this is why your New Charge showed \u2026110D while a \u2026110D already existed). Nothing else changed.' },
        { version: 'v3.72', date: '2026-08-22', notes: 'Tracker 4.0 — the finishing phase. (1) The Home now matches the approved design exactly: greeting with the date, an attention row with Approvals pending (live count), Paycheck decisions, Expiring documents, Unread messages and Notifications, quick actions (New claim, Add person, Upload document), a “This week’s payroll” snapshot with real numbers and the week’s net-to-pay total, and a Recent activity feed — all one tap from their screens. On phones the same dashboard stacks cleanly and a floating + button carries the quick actions. (2) Quieter, more professional tables: headers are calm uppercase labels instead of solid blue bars, the alternating stripes are gone, and the number tiles lost their left color bar. This completes the Tracker 4.0 redesign (v3.68–v3.72).' },
        { version: 'v3.71', date: '2026-08-22', notes: 'Tracker 4.0 — phase 4: the new Home. It now greets you by time of day and puts what needs your attention first: pending paycheck decisions, documents expiring within 30 days, unread messages and unread notifications, each one tap away. Quick actions (New claim, Add person, New invoice) sit at the top for managers. The number tiles below were slimmed — expiring documents and notifications moved up into the attention row instead of repeating.' },
        ...
        { version: 'v3.66', date: '2026-08-22', notes: 'Small polish to the side menu: when you expand “Profile photo” or “Security”, the options inside are now indented to the right with a connecting line, so it’s easy to see they belong to the section above.' },
        { version: 'v3.65', date: '2026-08-22', notes: 'Two refinements to the new look. (1) The side‑menu icons are now colorful — each section has its own purpose‑drawn icon in its own color (drawn graphics, so they stay crisp and colorful on every device, unlike emoji). (2) The bottom of the side menu was condensed: the account options are now grouped into just three items — “Profile photo”, “Security” (Change Password, Two‑Factor, and Sign out other devices), and “Sign Out” — so it is far less cluttered. Tap Profile photo or Security to expand its options. The Tracker logo and the language flags are unchanged, and the Messages emoji picker is unchanged.' },
        { version: 'v3.64', date: '2026-08-22', notes: 'A friendlier, more visual interface. (1) Every section in the side menu now has its own icon — Home 🏠, Fleet 🚚, Employees 👥, Claims 📑, Payroll 💰, Invoices 🧾, Settings ⚙️, Messages 💬 and so on — so you can find things at a glance. The main action buttons (Add Employee, Save Claim, Save Invoice, Sign In, and the rest) picked up matching icons too. The Tracker logo and the language flags are unchanged. (2) The Messages emoji picker now has the full emoji set, organised into category tabs (smileys, people, animals, food, activities, travel, objects, symbols, flags) with a search box and a “recently used” row that remembers your last picks on this device.' },
        { version: 'v3.63', date: '2026-08-22', notes: 'Optional profile photos. Anyone — employee, staff, contractor, provider — can now have a profile picture, and it is completely optional: a person without one simply shows a tidy coloured badge with their initials, so no card ever looks empty. You can set or remove YOUR OWN photo from the account menu (📷 Change photo / 🗑️ Remove photo), and managers can set or replace a photo on any person’s record from the person’s card in the Employees tab. Photos appear next to your name in the top bar, on every person in the Employees list, and on the record card. A picked photo is cropped to a square and shrunk automatically so it stays small and loads fast, and it is stored privately and company-scoped, reached only through the same secure, permission-checked path the rest of the app uses.' },
        { version: 'v3.62', date: '2026-08-21', notes: 'The Spanish language option now shows the flag of Spain (red-yellow-red) instead of the Mexican flag, on both the login screen and the Settings screen. Nothing else changes — the option is still labelled “Español” and switches the app to Spanish exactly as before.' },
        ...
        { version: 'v3.55', date: '2026-08-21', notes: 'Files now open inside the app instead of in a new browser tab. When you tap “View” on any photo, video or document — whether in a record’s Files or in a chat — it now opens in a viewer window right here in the app. Photos, videos, audio and PDFs display inline; any other kind of file (for example a Word or Excel document) shows its details with a Download button, since browsers can’t display those directly. The viewer has a Download button and a Close button (you can also press Esc or tap outside to close), images can be zoomed in and out with the − / + buttons or by holding Ctrl (⌘ on Mac) and scrolling, and it always shows who uploaded the file and the exact date and time it was uploaded. Nothing opens a new tab and nothing leaves the app.' },
        { version: 'v3.54', date: '2026-08-21', notes: 'Second internal release in a row with no change to how the app looks or works — again, the only visible difference is the version number shown above. The previous version repaired the automatic link that publishes new versions to the server; this one exists to confirm that the repair holds and repeats, rather than having happened to work once. Normal updates resume from here.' },
